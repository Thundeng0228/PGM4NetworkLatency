...
def test_dcs(latent_dim=70, batch_size=2, m_dim=int(120*120*0.9), num_training_epoch=1, lr=1e-4, initial_step_size=0.01,
             num_grad_iters=5, device=torch.device("cuda:0"), if_grad=True):
    ...
    step_size = Step_size().to(
        device)  # torch.nn.Parameter(torch.ones(1) * np.log(initial_step_size)).to(device) #* np.log(initial_step_size)
    ...
    policy.load_state_dict(torch.load('./PLB_policy_dmc.pth'))
    ...
    for epoch in pbar:
        for i, data in enumerate(training_data):
            ...
            measurement_original_data, indicator = measurement(original_data)
            ...
            measurement_original_data, _ = measurement(original_data)
            measurement_generated_data_initial, _ = measurement(generated_data_initial)
            measurement_generated_data_optimized, _ = measurement(generated_data_optimized)
            ...
            if True:
                ...
                E = (generated_data_optimized - original_data)[:, indicator]
                E2 = E.norm(dim=-1)
                # E2 = torch.mul(E, E).sum(dim=-1).sqrt()
                # O = torch.mul(original_data, original_data).sum(dim=-1).sqrt()
                O = original_data.norm(dim=-1)
                rse = torch.div(E2, O)
                myrse = rse.mean()
                # print(myrse)
                meanrse.append(myrse)
        print((meanrse[0]+meanrse[1])/2)
